            ...
            for (transaction &tx : blk_txs)
            {
                ...
                if (!oi_identification.identified_outputs.empty())
                {
                    ...
                    if (!mysql_transaction)
                    {
                        ...
                        mysql_transaction = unique_ptr<mysqlpp::Transaction>(
...
void TxSearch::set_searched_blk_no(uint64_t new_value)
{
...
bool TxSearch::still_searching()
{
    return continue_search;
}

void TxSearch::populate_known_outputs()
{
...
json TxSearch::find_txs_in_mempool(
...
void TxSearch::set_search_thread_life(uint64_t life_seconds)
{
